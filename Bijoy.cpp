...
void loadScore(int *score1, int *score2);
...
void loadScore(int *player1_score, int *player2_score) {
    FILE *file = fopen("save_score.txt", "r");
    if (file != NULL) {
        fscanf(file, "%d %d %d %d", player1_score, player2_score, minute,time);
        fclose(file);
        printf("Game score loaded.\n");
    } else {
        printf("No saved score found.\n");
    }
}
void saveScore(int score1, int score2, int minute, int second) {
    FILE *file = fopen("save_score.txt", "w");
    if (file != NULL) {
        fprintf(file, "%d %d %d %d\n", player1_score, player2_score, minute, second);
        fclose(file);
    } else {
...
void iKeyboard(unsigned char key)
{
    ...
    if (key == 's') {
        saveScore(player1_score, player2_score, minute, second);
        k = 0;
    }
    if (key == 'l') {
        loadScore(&player1_score, &player2_score);
        k = 1;
    }
}
