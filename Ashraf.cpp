...
void move_ball()
{
    ...
    if (currentPowerUpIcon.isActive) {
        if (hit(currentPowerUpIcon.x, currentPowerUpIcon.y, currentPowerUpIcon.width, currentPowerUpIcon.height, pic_ballx, pic_bally, ball_width, ball_width)) {
            ...
            currentPowerUpIcon.isActive = false; // Make the icon disappear

            // --- Apply the effect based on who hit it ---
            if (lastPlayerToTouchBall == 1) { // Player 1 gets the power-up
                if (collectedType == SUPER_KICK) {
                    ...
                    player2_effects.effectTimer = 30; // 3 seconds (30 * 100ms)
                } else if (collectedType == SIZE_DOWN) {
                    player2_effects.isSizeDown = true;
                    player2_effects.effectTimer = 50; // 5 seconds (50 * 100ms)
                }
            } else if (lastPlayerToTouchBall == 2) { // Player 2 gets the power-up
                if (collectedType == SUPER_KICK) {
